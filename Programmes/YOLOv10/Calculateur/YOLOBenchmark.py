from ultralytics import YOLO
import pandas as pd
import numpy as np
import os
import shutil

empty = False


# Charge les prédictions du modèle YOLO sur une image et renvoie le dataframe
def load_PRED_labels(model_path, image_path, train_num, nb_epochs, nb_batch):
    model = YOLO(model_path, task='detect')
    results = model(image_path)
    
    empty_test = results[0].to_df()

    if empty_test.empty:
        global empty
        empty = True
        results[0].save(f"ResultImage_e{nb_epochs}_b{nb_batch}_t{train_num}.jpg")
        data = []
        df = pd.DataFrame(data)

    else:
        results[0].save(f"ResultImage_e{nb_epochs}_b{nb_batch}_t{train_num}.jpg")
        results[0].save_txt(f"ResultLabels_e{nb_epochs}_b{nb_batch}_t{train_num}.txt")
        df = pd.read_csv(f"ResultLabels_e{nb_epochs}_b{nb_batch}_t{train_num}.txt", sep=' ', header=None, names=['class', 'x_center', 'y_center', 'width', 'height'])

    return df



def load_GT_labels(txt_path):
    df = pd.read_csv(txt_path, sep=' ', header=None, names=['class', 'x_center', 'y_center', 'width', 'height'])
    return df



# Convertit les dataframes en liste de dict (pour match_boxes)
def df_to_label_list(df):
    labels = []
    if not empty:
        for _, row in df.iterrows():
            bbox = [row['x_center'], row['y_center'], row['width'], row['height']]
            labels.append({
                'class': int(row['class']),
                'bbox': bbox
            })
    return labels




def bbox_iou(box1, box2):
    def to_corners(box):
        x, y, w, h = box
        return [x - w/2, y - h/2, x + w/2, y + h/2]
    
    b1 = to_corners(box1)
    b2 = to_corners(box2)

    xi1, yi1 = max(b1[0], b2[0]), max(b1[1], b2[1])
    xi2, yi2 = min(b1[2], b2[2]), min(b1[3], b2[3])
    inter_area = max(0, xi2 - xi1) * max(0, yi2 - yi1)

    b1_area = (b1[2] - b1[0]) * (b1[3] - b1[1])
    b2_area = (b2[2] - b2[0]) * (b2[3] - b2[1])
    union_area = b1_area + b2_area - inter_area
    return inter_area / union_area if union_area > 0 else 0




def match_boxes(gt_labels, pred_labels, iou_thresh=0.5):

    matches = []
    gt_matched = set()
    pred_matched = set()

    for i, gt in enumerate(gt_labels):
        for j, pred in enumerate(pred_labels):
            if gt['class'] == pred['class']:
                iou = bbox_iou(gt['bbox'], pred['bbox'])
                if iou >= iou_thresh:
                    matches.append((i, j, iou))
                    gt_matched.add(i)
                    pred_matched.add(j)

    return matches, gt_matched, pred_matched




def compute_metrics(gt_labels, pred_labels, inference_time_ms):
    n_gt = len(gt_labels)
    n_pred = len(pred_labels)
    matches, gt_matched, pred_matched = match_boxes(gt_labels, pred_labels)
    n_matched = len(matches)
    ious = [iou for _, _, iou in matches]

    # 1. Nombre d'individus détectés (25 points)
    rel_error = abs(n_pred - n_gt) / max(1, n_gt)
    score_count = 25 * (1 - min(rel_error / 0.5, 1))

    # 2. IoU moyen (25 points)
    iou_mean = np.mean(ious) if ious else 0
    score_iou = 25 * (iou_mean - 0.5) / 0.35 if iou_mean >= 0.5 else 0
    score_iou = min(max(score_iou, 0), 25)

    # 3. Précision (10 points)
    precision = n_matched / n_pred if n_pred > 0 else 0
    score_precision = 10 if precision >= 0.95 else 10 * precision if precision >= 0.1 else 0
    score_precision = min(max(score_precision, 0), 10)

    # 4. Rappel (10 points)
    recall = n_matched / n_gt if n_gt > 0 else 0
    score_recall = 10 if recall >= 0.95 else 10 * recall if recall >= 0.1 else 0
    score_recall = min(max(score_recall, 0), 10)

    # 5. mAP@0.5 (approximation) (10 points)
    ap = n_matched / (n_pred + n_gt - n_matched) if (n_pred + n_gt - n_matched) > 0 else 0
    score_map = 10 * ap if ap >= 0.1 else 0
    score_map = min(max(score_map, 0), 10)

    # 6. Temps d'inférence (10 points)
    if inference_time_ms < 15:
        score_time = 10
    elif inference_time_ms > 1000:
        score_time = 0
    else:
        score_time = 10 * (1 - (inference_time_ms - 15) / 900)

    # 7. Faux positifs (10 points)
    false_positives = n_pred - len(pred_matched)
    fpr = false_positives / max(1, n_pred)
    if fpr <= 0.05:
        score_fpr = 10
    elif fpr >= 0.3:
        score_fpr = 0
    else:
        score_fpr = 10 * (1 - (fpr - 0.05) / 0.25)

    # Score F1 (Affichage seulement)
    if recall + precision > 0:
        score_f1 = 2 * ((recall * precision) / (recall + precision))
    else:
        score_f1 = 0


    # Score final
    score_total = score_count + score_iou + score_precision + score_recall + score_map + score_time + score_fpr

    details = {
        'score_total': round(score_total, 2),
        'score_count': round(score_count, 2),
		'score_iou': round(score_iou, 2),
		'score_precision': round(score_precision, 2),
		'score_recall': round(score_recall, 2),
        'score_f1': round(score_f1, 2),
		'score_map': round(score_map, 2),
		'score_time': round(score_time, 2),
		'score_fpr': round(score_fpr, 2),
		'precision': round(precision, 3),
		'recall': round(recall, 3),
        'iou_mean': round(iou_mean, 3),
        'n_gt': n_gt,
		'n_pred': n_pred,
		'n_matched': n_matched,
		'inference_time_ms': inference_time_ms,
		'false_positives': false_positives,
		'fpr': round(fpr, 3)
		}
    return details



def save_metrics_to_txt(score_details, model_path, nb_epochs, nb_batch, id_num):
    file_name = f"Results/Result_e{nb_epochs}_b{nb_batch}_t{id_num}/ResultsResum_e{nb_epochs}_b{nb_batch}_t{id_num}.txt"
    os.makedirs(f"Results/Result_e{nb_epochs}_b{nb_batch}_t{id_num}", exist_ok=True)

    shutil.copyfile(model_path, f"Results/Result_e{nb_epochs}_b{nb_batch}_t{id_num}/Best0{id_num}.pt")

    with open(file_name, 'w') as f:
        f.write(f"              *** RESULTATS ***\n\nepochs={nb_epochs}, batch={nb_batch}, BDD version={bdd_version}, train ID={id_num}\n\n")
        f.write(f"Score global: {score_details['score_total']}/100\n\n")
        f.write("---- Détails des scores ----\n")
        f.write(f"Nombre individus: {score_details['score_count']}/25\n")
        f.write(f"IoU moyen: {score_details['score_iou']}/25 (valeur={score_details['iou_mean']})\n")
        f.write(f"Précision: {score_details['score_precision']}/10 (valeur={score_details['precision']})\n")
        f.write(f"Rappel: {score_details['score_recall']}/10 (valeur={score_details['recall']})\n")
        f.write(f"F1: {score_details['score_f1']}\n")
        f.write(f"mAP@0.5: {score_details['score_map']}/10\n")
        f.write(f"Temps d’inférence: {score_details['score_time']}/10 ({score_details['inference_time_ms']} ms)\n")
        f.write(f"Faux positifs: {score_details['score_fpr']}/10 (fpr={score_details['fpr']})\n\n")
        f.write("---- Statistiques brutes ----\n")
        f.write(f"Nombre total réels d'individus: {score_details['n_gt']}\n")
        f.write(f"Nombre total détectés d'individus: {score_details['n_pred']}\n")
        f.write(f"Invidus réels détectés: {score_details['n_matched']}\n")
        f.write(f"Faux positifs: {score_details['false_positives']}\n")


#----

image_path = 'Truth/truthImage.jpg'
gt_txt = 'Truth/truthImage.txt'


nb_epochs = 100
nb_batch = 32
bdd_version = 2
train_num = 10

inference_time_ms = 65

model_path = f'runs/detect/train{train_num}/weights/best.pt'

gt_labels = df_to_label_list(load_GT_labels(gt_txt))
pred_labels = df_to_label_list(load_PRED_labels(model_path, image_path, train_num, nb_epochs, nb_batch))

if empty == False:
    score_details = compute_metrics(gt_labels, pred_labels, inference_time_ms)
    save_metrics_to_txt(score_details, model_path, nb_epochs, nb_batch, train_num)
    shutil.move(f"ResultImage_e{nb_epochs}_b{nb_batch}_t{train_num}.jpg", f"Results/Result_e{nb_epochs}_b{nb_batch}_t{train_num}/")
    shutil.move(f"ResultLabels_e{nb_epochs}_b{nb_batch}_t{train_num}.txt", f"Results/Result_e{nb_epochs}_b{nb_batch}_t{train_num}/")

    print("\n#####################")
    print("Score global :", score_details['score_total'],"/100")
    print("#####################\n")
    print("Détail du score :")
    print("---------------------")
    for score in score_details:
        print(score, "=", score_details[score])

else:
    file_name = f"Results/Result_e{nb_epochs}_b{nb_batch}_t{train_num}/ResultsResum_e{nb_epochs}_b{nb_batch}_t{train_num}.txt"
    os.makedirs(f"Results/Result_e{nb_epochs}_b{nb_batch}_t{train_num}", exist_ok=True)

    shutil.copyfile(model_path, f"Results/Result_e{nb_epochs}_b{nb_batch}_t{train_num}/Best0{train_num}.pt")

    with open(file_name, 'w') as f:
        f.write(f"              *** RESULTATS ***\n\nepochs={nb_epochs}, batch={nb_batch}, BDD version={bdd_version}, train ID={train_num}\n\n")
        f.write(f"Score global: 0/100\n\n")
        f.write("---- Détails des scores ----\n")
        f.write("NULL\n")
        f.write("---- Statistiques brutes ----\n")
        f.write("NULL\n")


    print("\n#####################")
    print("Score global : 0/100")
    print("AUNCUNES ABEILLES N'A ETE DETECTE")
    print("#####################\n")